...
function renderGroup(groupData) {
    ...
    container._cyreg.cy.on('dbltap', 'node', function (evt) {
        ...
        // Cytoscape 컨테이너 위치 가져오기
        const cyContainer = container._cyreg.cy.container();
        const containerRect = cyContainer.getBoundingClientRect();
    
        // 더블탭한 위치 가져오기 (Cytoscape 내부 좌표 -> 화면 절대 좌표 변환)
        ...
        // 노드 관련 정보 가져오기
        const connectedEdges = node.connectedEdges(); // 연결된 엣지들
        const incomingEdges = connectedEdges.filter(edge => edge.target().id() === node.id());
        const outgoingEdges = connectedEdges.filter(edge => edge.source().id() === node.id());
        const neighbors = node.neighborhood('node'); // 이웃 노드
    
        // 연결된 엣지 정보 텍스트 생성
        const incomingText = incomingEdges.map(edge => {
            return `${edge.source().id()} -> ${edge.target().id()} (weight: ${edge.data('weight')})`;
        }).join('<br>');
    
        const outgoingText = outgoingEdges.map(edge => {
            return `${edge.source().id()} -> ${edge.target().id()} (weight: ${edge.data('weight')})`;
        }).join('<br>');
    
        // 이웃 노드 상세 정보 텍스트 생성
        const neighborText = neighbors.map(neighbor => {
            return `
                Event Name: ${neighbor.data('event_name') || 'N/A'}<br>
                Performer Name: ${neighbor.data('performer_name') || 'N/A'}<br>
                Timestamp: ${neighbor.data('time_stamp') || 'N/A'}
            `;
        }).join('<hr>');
        ...
        popup.style.resize = 'both'; // 크기 조정을 활성화
        popup.style.overflow = 'auto'; // 내용이 넘칠 경우 스크롤 추가
    
        // 팝업 내용 추가
        popup.innerHTML = `
            <h4 style="margin: 0 0 10px 0;">Node Information</h4>
            <p><strong>Event Name:</strong> ${node.data('event_name') || 'N/A'}</p>
            <p><strong>Performer Name:</strong> ${node.data('performer_name') || 'N/A'}</p>
            <p><strong>Timestamp:</strong> ${node.data('time_stamp') || 'N/A'}</p>
            <hr>
            <h4>Graph Details</h4>
            <p><strong>Incoming Edges:</strong></p>
            <p>${incomingText || 'None'}</p>
            <p><strong>Outgoing Edges:</strong></p>
            <p>${outgoingText || 'None'}</p>
            <p><strong>Neighbor Nodes:</strong></p>
            <div>${neighborText || 'None'}</div>
            <button id="close-popup" style="margin-top: 10px; background: #f44336; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Close</button>
        `;
    ...
    container._cyreg.cy.on('dbltap', 'edge', function (evt) {
        ...
        // 소스 노드 및 타겟 노드에 연결된 엣지 정보 가져오기
        const sourceConnectedEdges = sourceNode.connectedEdges().map(e => `${e.source().id()} -> ${e.target().id()}`).join('<br>');
        const targetConnectedEdges = targetNode.connectedEdges().map(e => `${e.source().id()} -> ${e.target().id()}`).join('<br>');
        ...
        // 팝업 내용 추가
        popup.innerHTML = `
            <h4 style="margin: 0 0 10px 0;">Edge Information</h4>
            <p><strong>Source:</strong> ${edge.data('source')}</p>
            <p><strong>Target:</strong> ${edge.data('target')}</p>
            <p><strong>Weight:</strong> ${edge.data('weight') || 'N/A'}</p>
            <hr>
            <h4>Source Node Details</h4>
            <p><strong>Node ID:</strong> ${sourceNode.id()}</p>
            <p><strong>Event Name:</strong> ${sourceNode.data('event_name') || 'N/A'}</p>
            <p><strong>Performer Name:</strong> ${sourceNode.data('performer_name') || 'N/A'}</p>
            <p><strong>Connected Edges:</strong><br>${sourceConnectedEdges || 'None'}</p>
            <hr>
            <h4>Target Node Details</h4>
            <p><strong>Node ID:</strong> ${targetNode.id()}</p>
            <p><strong>Event Name:</strong> ${targetNode.data('event_name') || 'N/A'}</p>
            <p><strong>Performer Name:</strong> ${targetNode.data('performer_name') || 'N/A'}</p>
            <p><strong>Connected Edges:</strong><br>${targetConnectedEdges || 'None'}</p>
            <button id="close-popup" style="margin-top: 10px; background: #f44336; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;">Close</button>
        `;
